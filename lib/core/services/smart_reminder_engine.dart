  ...
  static ReminderResult? evaluate({
    ...
    candidates.sort((a, b) {
      ...
      if (priorityCompare != 0) {
        return priorityCompare; // low index in enum means high priority in ordering if we sort carefully, wait:
      }
      // In Dart enum ReminderPriority { high, medium, low }:
      // high.index = 0, medium.index = 1, low.index = 2.
      // So sorting ascending (a.index compareTo b.index) correctly puts high (0) before medium (1) before low (2).
